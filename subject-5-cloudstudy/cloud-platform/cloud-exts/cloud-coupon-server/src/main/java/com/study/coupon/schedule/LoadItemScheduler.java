...
public class LoadItemScheduler extends IJobHandler {
    ...
    public ReturnT<String> execute(String arg0) throws Exception {
        ...
        for (TbCoupon tbCoupon : tbCoupons) {
            ...
            tokenKey.append("{")
                    .append(tbCoupon.getId())
                    .append("}.tokenlists");
            stringRedisTemplate.delete(tokenKey.toString());
            for (int i = 0; i < tbCoupon.getCouponSum(); i++) {
                // 优惠券都有对应的可抢购数值，这里的redis，是集群的
                stringRedisTemplate.opsForList().rightPush(tokenKey.toString(), String.valueOf(tbCoupon.getCouponSum()));
            }
        }
