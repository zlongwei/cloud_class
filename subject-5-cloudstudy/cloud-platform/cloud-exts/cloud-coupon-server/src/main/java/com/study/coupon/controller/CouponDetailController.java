...
public class CouponDetailController {
    ...
    public ObjectRestResponse lockCoupon(Long couponDetailId, Long orderId, Long orderUserId) {
        ObjectRestResponse response = new ObjectRestResponse();
        // 不是同一个用户，提示非法请求
        String loginUserId = BaseContextHandler.getUserID();
        // 测试代码
        // loginUserId = "99999";
        if (!loginUserId.equals(String.valueOf(orderUserId))) {
            // 记录日志
    ...
    public ObjectRestResponse releaseCoupon(Long couponDetailId, Long orderId, String orderUserId) {
        ObjectRestResponse response = new ObjectRestResponse();
        // 不是同一个用户，提示非法请求
        String loginUserId = BaseContextHandler.getUserID();
        // 测试代码
    ...
    public TableResultResponse getCouponDetailByUserId() {
        ...
        for (TbCouponDetail tbCouponDetail : tbCouponDetails) {
            ...
            tbCouponDetailVO.setId(tbCouponDetail.getId());
